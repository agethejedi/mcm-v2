...
/* ============================================================
   ...
   MVP: Dow-only trading, cohort auto-assigned by system
   ============================================================ */
...
const VALID_STATUS = new Set(["open", "executed"]);
...
/* ============================================================
   MVP execution rules
   - market: execute immediately at submitted current_price
   - limit/stop/stop_limit/trailing_stop:
     * if trigger condition met from submitted current_price, execute now
     * else create open order
   ============================================================ */
...
function evaluateExecution({
  ...
  if (orderType === "trailing_stop") {
    // MVP simplification:
    // If trailing stop is submitted, store as open unless user provided a current price
    // that already violates an immediate derived stop against itself, which it won't.
    // So initial behavior is open.
...
async function handlePost(request, env) {
  try {
    ...
    // Buying power check for immediately executed buys
    ...
    // Position check for immediately executed sells
    ...
    // For open sell orders, also prevent overselling beyond current executed shares.
    ...
    const insert = await env.DB
      ...
      .bind(
        ...
        execution.status === "executed" ? new Date().toISOString() : null
      )
      .run();
